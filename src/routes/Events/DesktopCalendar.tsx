...
function CurrentDate({ day, info }: DateBaseProps) {
  ...
  return (
    <div
      className="
      p-2 flex flex-col items-center justify-center bg-blue-100 rounded-md shadow-sm hover:shadow-md transition-all w-full
...
export default function DesktopCalendar({
  ...
  month: number;
     ...
     const info = calculateEventMap(eventList);
      ...
      <div className="grid grid-cols-7 gap-2 rounded-lg">
        {days.map((x, i) => (
          <DateBuilder
            month={month}
            day={x}
            info={info.get(`${x.getMonth()}/${x.getDate()}`) ?? []}
            key={i}
          />
        ))}
      </div>
